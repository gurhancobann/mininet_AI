...
class NsfnetTopo(Topo):
	...
	def build(self, **params):
		...
		linkOptns2=dict(delay='25ms',bw=3, loss=5, max_queue_size=1000, use_htb=True)
	
		self.addLink(s1, s2, **linkOptns2)
		self.addLink(s1, s3, **linkOptns1)
...
def startNetwork():
	...
	floodlightRestApi.addACL(net.getNodeByName("h2").IP(),net.getNodeByName("h1").IP(),"deny")
	floodlightRestApi.addACL(net.getNodeByName("h3").IP(),net.getNodeByName("h1").IP(),"deny")
	#net.pingAll() 
	#d
	...
	psnr, ssim_first, ssim_second=calcPsnrSsim("h1")
	dosya_adi=videoSource
	kayit=f"records/h1/input.ts"
	boyutana=os.stat(dosya_adi).st_size
	boyutkayit=os.stat(kayit).st_size
	print(f"boyut ana: {boyutana}\nboyut kayıt :{boyutkayit}\nfark: {boyutana-boyutkayit}\nkayıp oran:{(((boyutana-boyutkayit)/boyutana)*100)}")
	dataRow={"psnr":psnr,"ssim_first":ssim_first,"ssim_second":ssim_second,"kaynak":boyutana,"kayitboyut":boyutkayit,"kayip":(boyutana-boyutkayit),"kayip_orani":(((boyutana-boyutkayit)/boyutana)*100),"devreye_giris":30}
...
def yayin(host):
	...
	floodlightRestApi.addACL(net.getNodeByName("h2").IP(),net.getNodeByName("h1").IP(),"allow")
	floodlightRestApi.addACL(net.getNodeByName("h3").IP(),net.getNodeByName("h1").IP(),"deny")
	port="1234"
	senderNode1, senderNode2=net.getNodeByName("h2"), net.getNodeByName("h3")
	receiverNode=net.getNodeByName(host)
	dst_host_ipv4=receiverNode.IP()
	senderCommand1=f"ffmpeg -re -i {videoSource} -c copy -f mpegts udp://{dst_host_ipv4}:{port}"
	senderCommand2=f"ffmpeg -re -i {videoSource} -c copy -f mpegts udp://{dst_host_ipv4}:{port}"
	receiverURL=f"udp://{dst_host_ipv4}:{port}"
	receiverNode.cmd(f"mkdir records/{host}")
	receiverCommand=f"ffmpeg -i {receiverURL} -c copy records/{host}/input.ts"
	receiverThread=(HostCommand(receiverNode, receiverCommand))
	receiverThread.daemon=True
	receiverThread.start()
	senderThread1=(HostCommand(senderNode1, senderCommand1))
	...
	floodlightRestApi.clearACL()
	floodlightRestApi.addACL(net.getNodeByName("h2").IP(),net.getNodeByName("h1").IP(),"deny")
	floodlightRestApi.addACL(net.getNodeByName("h3").IP(),net.getNodeByName("h1").IP(),"allow")
	senderThread2.join()
